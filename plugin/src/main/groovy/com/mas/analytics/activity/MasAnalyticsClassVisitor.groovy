...
class MasAnalyticsClassVisitor extends ClassVisitor implements Opcodes {
    ...
    MasAnalyticsClassVisitor(final ClassVisitor classVisitor) {
    ...
    void visit(int version, int access, String name, String signature, String superName, String[] interfaces) {
        ...
        Logger.info("anlay 开始扫描类：${mClassName}")
        Logger.info("anlay 类详情：version=${version};\taccess=${Logger.accCode2String(access)};\tname=${name};\tsignature=${signature};\tsuperName=${superName};\tinterfaces=${interfaces.toArrayString()}\n")
    }
    ...
    void visitEnd() {
        ...
        if (MasAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
            MethodVisitor mv
            // 添加剩下的方法，确保super.onHiddenChanged(hidden);等先被调用
            Iterator<Map.Entry<String, MasAnalyticsMethodCell>> iterator = MasAnalyticsHookConfig.sFragmentMethods.entrySet().iterator()
            while (iterator.hasNext()) {
                Map.Entry<String, MasAnalyticsMethodCell> entry = iterator.next()
                String key = entry.getKey()
                MasAnalyticsMethodCell methodCell = entry.getValue()
                ...
                visitMethodWithLoadedParams(mv, Opcodes.INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, methodCell.agentName, methodCell.agentDesc, methodCell.paramsStart, methodCell.paramsCount, methodCell.opcodes)
                mv.visitInsn(Opcodes.RETURN)
                mv.visitMaxs(methodCell.paramsCount, methodCell.paramsCount)
                mv.visitEnd()
                mv.visitAnnotation("Lcom/qiudaoyu/monitor/data/SensorsDataInstrumented;", false)
            }
        }

        Logger.info("anlay 结束扫描类：${mClassName}\n")
    }

    static boolean shouldProcessMethod(String className, String name) {
        if ("<init>".equals(name) || "<clinit>".equals(name) || name.startsWith('access$') || name.startsWith('invokeMethodByIndex'))
            return false
    ...
    MethodVisitor visitMethod(int access, String name, String desc, String signature, String[] exceptions) {
        ...
        if (!shouldProcessMethod(mClassName, name)) {
            return methodVisitor
        }


        methodVisitor = new MasAnalyticsDefaultMethodVisitor(methodVisitor, access, name, desc) {
            ...
            void visitEnd() {
                ...
                if (isHasTracked) {
                    visitAnnotation("Lcom/qiudaoyu/monitor/data/SensorsDataInstrumented;", false)
                    Logger.info("anlay Hooked method: ${name}${desc}\n")
                }
            }
            ...
            protected void onMethodEnter() {
                ...
                if (!(MasAnalyticsUtil.isPublic(access) && !MasAnalyticsUtil.isStatic(access))) {
                    return
                }
                ...
                if (mClassName == 'com/facebook/react/uimanager/NativeViewHierarchyManager') {
                    if (nameDesc == 'setJSResponder(IIZ)V') {
                        ...
                        methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackRN", "(Ljava/lang/Object;IIZ)V", false)
                        isHasTracked = true
                        return
                    }
                }
                 ...
                 */
                if (MasAnalyticsUtil.isInstanceOfFragment(mSuperName)) {
                    MasAnalyticsMethodCell sensorsAnalyticsMethodCell = MasAnalyticsHookConfig.sFragmentMethods.get(nameDesc)
                    if (sensorsAnalyticsMethodCell != null) {
                        visitedFragMethods.add(nameDesc)
                        visitMethodWithLoadedParams(methodVisitor, Opcodes.INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, sensorsAnalyticsMethodCell.agentName, sensorsAnalyticsMethodCell.agentDesc, sensorsAnalyticsMethodCell.paramsStart, sensorsAnalyticsMethodCell.paramsCount, sensorsAnalyticsMethodCell.opcodes)
                        isHasTracked = true
                    }
                }
                 ...
                 */
                if (MasAnalyticsUtil.isTargetMenuMethodDesc(nameDesc)) {
                    methodVisitor.visitVarInsn(ALOAD, 0)
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackMenuItem", "(Ljava/lang/Object;Landroid/view/MenuItem;)V", false)
                    isHasTracked = true
                    isViewTracked = true
                    return
                }
                ...
                if (nameDesc == 'onDrawerOpened(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackDrawerOpened", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    return
                } else if (nameDesc == 'onDrawerClosed(Landroid/view/View;)V') {
                    methodVisitor.visitVarInsn(ALOAD, 1)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackDrawerClosed", "(Landroid/view/View;)V", false)
                    isHasTracked = true
                    isViewTracked = true
                    return
                }
                ...
                if (mClassName == 'android/databinding/generated/callback/OnClickListener') {
                    if (nameDesc == 'onClick(Landroid/view/View;)V') {
                        methodVisitor.visitVarInsn(ALOAD, 1)
                        methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackViewOnClick", "(Landroid/view/View;)V", false)
                        isHasTracked = true
                        isViewTracked = true
                        return
                    }
                ...
                if (nameDesc == 'onItemSelected(Landroid/widget/AdapterView;Landroid/view/View;IJ)V' || nameDesc == "onListItemClick(Landroid/widget/ListView;Landroid/view/View;IJ)V") {
                    ...
                    methodVisitor.visitVarInsn(ILOAD, 3)
                    methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackListView", "(Landroid/widget/AdapterView;Landroid/view/View;I)V", false)
                    isHasTracked = true
                    isViewTracked = true
                    return
                }
                ...
                if (mInterfaces != null && mInterfaces.length > 0) {
                    MasAnalyticsMethodCell sensorsAnalyticsMethodCell = MasAnalyticsHookConfig.sInterfaceMethods.get(nameDesc)
            ...
            protected void onMethodExit(int opcode) {
                if (isViewTracked) {
                    methodVisitor.visitMethodInsn(INVOKESTATIC, MasAnalyticsHookConfig.sSensorsAnalyticsAPI, "trackViewOnClickEnd", "()V", false)
                }
            ...
            AnnotationVisitor visitAnnotation(String s, boolean b) {
                if (s == 'Lcom/qiudaoyu/monitor/data/SensorsDataTrackViewOnClick;') {
                    isSensorsDataTrackViewOnClickAnnotation = true
                    Logger.info("anlay 发现 ${name}${desc} 有注解 @SensorsDataTrackViewOnClick")
                }

                if (s == 'Lcom/qiudaoyu/monitor/data/SensorsDataIgnoreTrackOnClick;') {
                    isSensorsDataIgnoreTrackOnClick = true
                    Logger.info("anlay 发现 ${name}${desc} 有注解 @SensorsDataIgnoreTrackOnClick")
                }

                if (s == 'Lcom/qiudaoyu/monitor/data/SensorsDataInstrumented;') {
